...
router.post('/citas', async (req, res) => {
  ...
  try {
    ...
    // Resolver idempleado del tatuador para hacer match con la suscripción push
    db.query(`SELECT idempleado FROM tatuador WHERE idtatuador = $1`, [idTatuador])
      .then(r => {
        if (r.rows.length) {
          notificarUsuario(2, r.rows[0].idempleado, {
...
router.put('/cotizaciones/:id/revisar', async (req, res) => {
  try {
    ...
    db.query(
      ...
      if (r.rows.length) {
        notificarUsuario(1, r.rows[0].idcliente, {
...
router.put('/cotizaciones/:id/rechazar', async (req, res) => {
  ...
  try {
    ...
    if (idClienteRech) {
      notificarUsuario(1, idClienteRech, {
