...
// GET /api/push/debug?idTipoUsuario=X&idUsuario=Y — diagnóstico completo
router.get('/push/debug', async (req, res) => {
  ...
  if (!idTipoUsuario || !idUsuario) {
    resultado.error = 'Falta idTipoUsuario o idUsuario en query params';
    return res.json(resultado);
  }

  try {
    const subs = await db.query(
      `SELECT idsubscription, LEFT(endpoint,60) AS endpoint_preview, activo, fecha_registro
       FROM push_subscription WHERE idtipo_usuario = $1 AND idusuario = $2`,
      [parseInt(idTipoUsuario), parseInt(idUsuario)]
    );
    resultado.subscripciones = subs.rows;

    const notifs = await db.query(
      `SELECT pn.idnotificacion, pn.titulo, pn.enviada, pn.error, pn.fecha_envio
       FROM push_notificacion pn
       JOIN push_subscription ps ON pn.idsubscription = ps.idsubscription
       WHERE ps.idtipo_usuario = $1 AND ps.idusuario = $2
       ORDER BY pn.fecha_envio DESC LIMIT 10`,
      [parseInt(idTipoUsuario), parseInt(idUsuario)]
    );
    resultado.ultimas_notificaciones = notifs.rows;
  } catch (err) {
